...
pub enum DataKey {
    ...
    PendingAdmin,
    /// Pending controller address for two-step controller rotation, keyed by program_id.
...
impl ProgramEscrowContract {
    ...
    fn batch_payout_internal(
        ...
        // 3.  Paused — release_paused blocks all payouts (deterministic, before auth)
        // 3b. Dispute guard
        ...
        // Step 3: pause check runs before authorization so a paused contract
        // deterministically rejects all callers with the same error.
        if Self::check_paused(&env, symbol_short!("release")) {
            panic!("Funds Paused");
        }
        ...
        for i in 0..recipients.len() {
            ...
            let pay_fee = Self::combined_fee_amount(gross, cfg.payout_fee_rate, cfg.payout_fixed_fee, cfg.fee_enabled);
            let net = gross.checked_sub(pay_fee).filter(|&v| v > 0)
        ...
        if let Some(key) = idempotency_key {
            Self::store_idempotency_record(&env, key, symbol_short!("batchpay"), updated_data.program_id.clone(), total_payout, recipients.len() as u32, executor);
        }
